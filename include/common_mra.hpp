...
extern double*     avg_task_exec_map;
extern double*     avg_task_exec_reduce;
...
extern struct mra_config_s {
    double          mra_chunk_size;
    double          grid_average_speed;
    double          grid_cpu_power;
    int             mra_chunk_count;
    int             mra_chunk_replicas;
    unsigned int    mra_heartbeat_interval;
    unsigned int    amount_of_tasks_mra[2];
    ...
    int            mra_map_output;
    /*OLD msg_host_t*    workers_mra;*/
    simgrid::s4u::Host** workers_mra;
} config_mra;


/*NEW*/typedef Task_MRA* mra_task_t;


extern struct mra_job_s {
    int           finished;
    int           tasks_pending[2];
    int*          task_instances[2];
    int*          task_status[2];
    /*OLD msg_task_t**  task_list[2];*/
    mra_task_t**  task_list[2];
    size_t**      map_output;
 ...
 * @return The MSG status of the operation.
 */
/*CHANGED*/void send_mra_sms (const char* str, const char* mailbox);
 ...
 */
/*CHANGED*/mra_task_t receive (/*OLD msg_task_t* msg,*/ const char* mailbox);
 ...
 */
/*CHANGED*/int mra_message_is (mra_task_t msg, const char* str);
