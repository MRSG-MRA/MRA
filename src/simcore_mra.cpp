...
struct mra_fd_s mra_f_detec_f;
struct mra_ftsys_s mra_ftsys_f, mra_ftm_done_f;
...
static void run_mra_simulation (const char* platform_file, const char* deploy_file, const char* mra_config_file, const char* vc_file_name);
...
int MRA_main (const char* plat, const char* depl, const char* conf, const char* trace_vc)
{
    ...
    run_mra_simulation (plat, depl, conf, trace_vc);
...
static void run_mra_simulation (const char* platform_file, const char* deploy_file, const char* mra_config_file, const char* vc_file_name)
{
    /*OLD msg_error_t  res_mra = MSG_OK;*/
    simgrid::s4u::Engine* e = simgrid::s4u::Engine::get_instance();

    read_mra_config_file (mra_config_file);

    /*OLD MSG_create_environment (platform_file);*/
    ...
    e->register_function("master_mra", master_mra);
    e->register_function("worker_mra", worker_mra);
    e->load_deployment(deploy_file);

    init_mr_mra_config (mra_config_file);

    /*OLD res_mra = MSG_main ();*/
    e->run();
...
static void read_mra_config_file (const char* file_name)
{
    ...
    else if ( strcmp (property, "mra_map_output") == 0 )
	{
	    fscanf (file, "%d", &config_mra.mra_map_output);
	}
...
static void init_mra_config (void)
{
    ...
    for(unsigned int i = 0; i < host_list.size(); i++){
        ...
        for(unsigned int j = 0; j < actor_list.size(); j++){
            ...
            if( strcmp (actor->get_cname(), "worker_mra") == 0 )
                config_mra.mra_number_of_workers++; 
                
        }
    }

    config_mra.workers_mra = xbt_new (/*OLD msg_host_t*/ simgrid::s4u::Host*, config_mra.mra_number_of_workers);
    ...
    for(unsigned int i = 0; i < host_list.size(); i++){
        ...
        for(unsigned int j = 0; j < actor_list.size(); j++){
            ...
            if ( strcmp (actor->get_cname(), "worker_mra") == 0 ){
                ...
                host->set_property("WID", std::to_string(mra_wid));
                /*OLD MSG_host_set_data (host, (void*)wi);*/

                /* Add the worker's cpu power to the grid total. */
                config_mra.grid_cpu_power += host->get_speed();
                mra_wid++;
                /*OLD config_mra.grid_cpu_power += MSG_host_get_speed (host);*/
...
static void init_job_mra (void)
{
    ...
    job_mra.task_list[MRA_MAP] = xbt_new0 (mra_task_t*, config_mra.amount_of_tasks_mra[MRA_MAP]);
    for (unsigned int i = 0; i < config_mra.amount_of_tasks_mra[MRA_MAP]; i++)
	  job_mra.task_list[MRA_MAP][i] = xbt_new0 (mra_task_t, MAX_SPECULATIVE_COPIES);
    ...
    for (int i = 0; i < config_mra.mra_number_of_workers; i++)
    ...
    job_mra.task_list[MRA_REDUCE] = xbt_new0 (mra_task_t*, config_mra.amount_of_tasks_mra[MRA_REDUCE]);
    for (unsigned int i = 0; i < config_mra.amount_of_tasks_mra[MRA_REDUCE]; i++)
	  job_mra.task_list[MRA_REDUCE][i] = xbt_new0 (mra_task_t, MAX_SPECULATIVE_COPIES);
...
static void free_mra_global_mem (void)
{
    for (int i = 0; i < config_mra.mra_chunk_count; i++)
