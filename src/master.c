...
static int is_straggler (enum phase_e phase, m_host_t worker); 
// static int is_straggler (m_host_t worker);
static int task_time_elapsed (m_task_t task);
static void set_speculative_tasks (enum phase_e phase, m_host_t worker);
...
static double task_cost (enum phase_e phase, size_t tid, m_host_t worker);
...
int master (int argc, char* argv[])
{
    ...
    while (job.tasks_pending[REDUCE] > 0)
    {
	...
	if (message_is (msg, SMS_HEARTBEAT))
	{
	  ...
	  set_speculative_tasks (MAP, worker);
	  set_speculative_tasks (REDUCE, worker);
	   ...
	   /* if (is_straggler (worker))
	    ...
	    //else
	    if (is_straggler (MAP, worker) == 0 )  {
	    
		if (heartbeat->slots_av[MAP] > 0 && dist_bruta [wid] > 0) //if (heartbeat->slots_av[MAP] > 0)
		    send_map_to_worker (worker); }
		  
            if (is_straggler (REDUCE, worker) == 0){ 		  

		if (heartbeat->slots_av[REDUCE] > 0 && dist_bruta [wid] > 0) //if (heartbeat->slots_av[REDUCE] > 0)
...
/* static int is_straggler (m_host_t worker)
{
    ...
    //if (MSG_get_host_speed (worker) < config.grid_average_speed && task_count > 0)
     if (MSG_get_host_speed (worker) < avg_task_exec[wid] && task_count > 0)
	return 1;
...
static void set_speculative_tasks (enum phase_e phase, m_host_t worker)
{
	...
	for (tid = 0; tid < config.number_of_maps; tid++)
	{
		...
		if (ti->wid == wid && dist_bruta [wid] > 0 && task_time_elapsed (job.task_list[MAP][0][tid]) > 60)
		{
    ...
    if (is_straggler (REDUCE, worker) == 1 ){
	...
	for (tid = 0; tid < config.number_of_reduces; tid++)
	{
		...
		if (ti->wid == wid && dist_bruta [wid] > 0 && task_time_elapsed (job.task_list[REDUCE][0][tid]) > 60)
		{
...
static void send_task (enum phase_e phase, size_t tid, size_t data_src, m_host_t dest)
{
    ...
    {
	case MAP:
	    task_state = job.task_state[MAP];
	    task_list = job.task_list[MAP];
	    break;

	case REDUCE:
	    task_state = job.task_state[REDUCE];
	    task_list = job.task_list[REDUCE];
	    break;
    }

    cpu_required = task_cost (phase, tid, dest);
    ...
    if (task_state[tid] != T_STATE_TIP_SLOW)
	task_state[tid] = T_STATE_TIP;

    w_heartbeat[wid].slots_av[phase]--;

    for (i = 0; i < MAX_SPECULATIVE_COPIES; i++)
    {
	if (task_list[i][tid] == NULL)
	{
	    task_list[i][tid] = task;
	    break;
	}
    }
...
static double task_cost (enum phase_e phase, size_t tid, m_host_t worker)
{
    ...
    {
	case MAP:
	    cost = config.cpu_required_map;
	    break;

	case REDUCE:
	    cost = config.cpu_required_reduce;
	    break;
    }
 ...
 * @param  ti  The task info of any task copies.
 */
static void finish_all_task_copies (task_info_t ti)
{
    int     i;
	...
	if (job.task_list[phase][i][tid] != NULL)
	{
	   ...
	   // MSG_task_destroy (job.task_list[phase][i][tid]);
