...
struct mra_dfs_het_s mra_dfs_het_f;
struct mra_dfs_het_s* mra_dfs_dist;
char**  chunk_owner_mra;

static void send_mra_data (mra_task_t msg);
...
void default_mra_dfs_f (char** mra_dfs_matrix, size_t chunks, size_t workers_mra, int replicas)
{
    ...
    */
    ...
    for (owner = 0; owner < config_mra.mra_number_of_workers; owner++)
    {
        ...
        /*OLD mra_dfs_dist->speed = MSG_host_get_speed(config_mra.workers_mra[owner]);*/
        mra_dfs_dist->speed = config_mra.workers_mra[owner]->get_speed();
        ...
        /*OLD fprintf (log, " %s , ID: %d \t Cap_Per: %f \t te_ex: %g \t Dist_B: %u \t Soma: %u \t Pre_ex: %g \t Tem_Cor: %g\n",
        ...
        fprintf (log, " %s , ID: %d \t Cap_Per: %f \t te_ex: %g \t Dist_B: %u \t Soma: %u \t Pre_ex: %g \t Tem_Cor: %g\n",
                 config_mra.workers_mra[owner]->get_cname(), owner, mra_dfs_dist[owner].p_cap_wid,mra_dfs_dist[owner].task_exec[MRA_MAP],
                 mra_dfs_dist[owner].dist_bruta, mra_dfs_het_f.dist_sum,mra_dfs_dist[owner].prev_exec[MRA_MAP], mra_dfs_dist[owner].temp_corr[MRA_MAP]);
    }
    fclose (log);
    ...
    for (owner = 0; owner < config_mra.mra_number_of_workers; owner++)
    {
        ...
        /*OLD fprintf (log, " %s , ID: %d \t Dist_Bruta: %u \t Reduces: %u \t Te_exec_Min: %g \t Tarefas_Reduce %u \t Prev_exec_reduce %g \n",
        ...
        fprintf (log, " %s , ID: %d \t Dist_Bruta: %u \t Reduces: %u \t Te_exec_Min: %g \t Tarefas_Reduce %u \t Prev_exec_reduce %g \n",
                 config_mra.workers_mra[owner]->get_cname(), owner,mra_dfs_dist[owner].dist_bruta, tasks_reduce[owner], min_te_exec, tot_tasks_reduce, prev_exec_reduce[owner]);
    }
    fclose (log);
    ...
    for (owner = 0; owner < config_mra.mra_number_of_workers; owner++) //for each owner in machine
    {
        dist=0;
        total_chunk=0;
        while (dist < mra_dfs_dist[owner].dist_bruta) // while group number < owner group number
        {
...
//                    fprintf (log,"dist: %u \t dist_b:%u \t ID: %zu \t chunk: %zu \t total_chunk: %u \n",dist, mra_dfs_dist[owner].dist_bruta, owner, chunk, total_chunk);
 ...
 //   mra_replica_f();
    ...
    for (chunk = 0; chunk < config_mra.mra_chunk_count; chunk++)
    {
        ...
        fprintf (log, "Affinity chunks owned: %d = %d \n", chunk, mra_affinity[chunk]);
    }
    fclose (log);
            ...
            if (chunk_owner_mra[chunk][owner] == 1 && mra_dfs_dist[owner].dist_bruta != 0)
            {
                ...
                for (i = config_mra.mra_number_of_workers-1; i >= 0 ; i--)
                {
                  if ((mra_affinity[chunk] < config_mra.mra_chunk_replicas) &&  owner != i
                       ...
                       {
  		                    fprintf (log,"Owner: %d \t chunk: %d \t Replica to:%d \t DistBruta: %d \n",owner,chunk, i, mra_dfs_dist[owner].dist_bruta);
            ...
            if (chunk_owner_mra[chunk][owner] == 1 && mra_dfs_dist[owner].dist_bruta !=0)
            {
                while (mra_affinity[chunk] < config_mra.mra_chunk_replicas)
                {
                    ...
                    if (mra_dfs_dist[owner].dist_bruta != mra_dfs_dist[j].dist_bruta && chunk_owner_mra[chunk][j] == 0 && mra_dfs_dist[j].dist_bruta!=0 )
                    {
                        ...
                        {
                            fprintf (log,"Owner: %d \t chunk: %d \t Replica to:%d \n",owner,chunk, j);
...
void min_max_f(int adjust, int chunk_num, int fault_mode)
{
    ...
    for (owner = 0; owner < config_mra.mra_number_of_workers; owner++)
    {
        ...
        /*OLD fprintf (log, " %s , ID: %d \t Dist_Recalc: %u \t Soma: %u \t Chunks %u \t Pre_ex= %g \t Tem_Cor= %g\n ",
        ...
        fprintf (log, " %s , ID: %d \t Dist_Recalc: %u \t Soma: %u \t Chunks %u \t Pre_ex= %g \t Tem_Cor= %g\n ",
                 config_mra.workers_mra[owner]->get_cname(),owner,mra_dfs_dist[owner].dist_bruta,dist_sum, chunk_num, mra_dfs_dist[owner].prev_exec[MRA_MAP],mra_dfs_dist[owner].temp_corr[MRA_MAP]);
    }
    fclose (log);
    /**
    ...
    */
    ...
    while ((minimal_task < dist_min) && tot_sum < config_mra.mra_number_of_workers )
    {
        ...
        for (owner = 0; owner < config_mra.mra_number_of_workers ; owner++)
        {
            ...
            /*OLD fprintf (log, " %s , ID: %d \t Dist_Recalc: %u \t Soma: %u \t Te_exec = %g \t Pre_ex= %g \t Tem_Cor= %g\n",
            ...
            fprintf (log, " %s , ID: %d \t Dist_Recalc: %u \t Soma: %u \t Te_exec = %g \t Pre_ex= %g \t Tem_Cor= %g\n",
                     config_mra.workers_mra[owner]->get_cname(),owner,mra_dfs_dist[owner].dist_bruta,dist_sum,mra_dfs_dist[owner].task_exec[MRA_MAP],mra_dfs_dist[owner].prev_exec[MRA_MAP],mra_dfs_dist[owner].temp_corr[MRA_MAP]);
        }
        fclose (log);
        dist_min = min_max;
...
/** @brief Clean all chunks already assigned to worker */
void mra_vc_clean_rpl (int owner)
{
    int 		chunk;
    FILE*    	log;
    int         counter;
    ...
    for (chunk = 0; (chunk < config_mra.mra_chunk_count) ; chunk++)
    {
        chunk_owner_mra[chunk][owner] = 0;
    }


    log = fopen ("new_chunks.log", "w");
    xbt_assert (log != NULL, "Error creating log file.");
    for (owner = 0; owner < config_mra.mra_number_of_workers; owner++)
    {
        fprintf (log, "worker %06d | ", owner);
...
void mra_vc_task_assing (int owner, int chunk)
{
    FILE*    		log;
    int         counter;
...
size_t find_random_mra_chunk_owner (int cid)
{
    ...
    {
        if (chunk_owner_mra[cid][mra_wid] && behavior[mra_wid] != VC_FAILURE )
        {
            owner = mra_wid;
    ...
    if(behavior[mra_wid]==NEW_WID) // NEW_WID){
    {
...
void data_node_mra (/*OLD int argc, char* argv[]*/)
{
    ...
    /*OLD sprintf (mailbox, DATANODE_MRA_MAILBOX, get_mra_worker_id (MSG_host_self ()));*/
    sprintf (mailbox, DATANODE_MRA_MAILBOX, get_mra_worker_id (simgrid::s4u::Host::current()));
...
*/
static void send_mra_data (/*OLD msg_task_t msg*/ mra_task_t msg)
{
    char         mailbox[MAILBOX_ALIAS_SIZE];
    double       data_size;
    size_t       my_id;
    ...
    /*OLD sprintf (mailbox, TASK_MRA_MAILBOX, get_mra_worker_id (MSG_task_get_source (msg)), MSG_process_get_PID (MSG_task_get_sender (msg)));*/

    my_id = get_mra_worker_id(simgrid::s4u::Host::current());
    sprintf (mailbox, TASK_MRA_MAILBOX,
	    get_mra_worker_id (msg->getSource()),
	    msg->getSender()->get_pid());
    
    if (mra_message_is (msg, SMS_GET_MRA_CHUNK))
    {
        /*OLD MSG_task_dsend (MSG_task_create ("DATA-C", 0.0, config_mra.mra_chunk_size, NULL), mailbox, NULL);*/
        send("DATA-C", 0.0, config_mra.mra_chunk_size, NULL, mailbox);
    }
    else if (mra_message_is (msg, SMS_GET_INTER_MRA_PAIRS))
    {
        ...
        /*OLD MSG_task_dsend (MSG_task_create ("DATA-IP", 0.0, data_size, NULL), mailbox, NULL);*/
        send("DATA-IP", 0.0, data_size, NULL, mailbox);
    }
